...
# ---------------------------------------------------------------------------
# Legacy: Chandra OCR (kept for reference, no longer used in pipeline)
# ---------------------------------------------------------------------------

def run_chandra_ocr(tables_dir, output_dir, method="hf", chandra_cmd=None):
    """
    [LEGACY] Run chandra OCR on table images.
    Writes HTML per image to output_dir/page_N/page_N.html.
    Use html_tables_to_csv.py to convert the HTML output to TSV.
    Replaced by run_claude_ocr() in the current pipeline.
    """
    tables_dir = os.path.abspath(tables_dir)
    output_dir = os.path.abspath(output_dir)
    os.makedirs(output_dir, exist_ok=True)
...
def _iter_page_htmls(table_data_dir):
    """[LEGACY] Yield paths to page_N.html under table_data_dir (chandra output)."""
